...
class GrdMiddlewareClient:
    ...
    def __init__(self, modbus_driver: ModbusTcpDriver, default_unit_id: int, register_count: int, refresh_interval: int = 2000):
        ...
        self._active_grd_data = None        # Cache de GRDs activos
        self._last_grd_data_refresh = 0     # Tiempo del ultimo refresh

    def _refresh_grd_data(self):
        """Refresca la lista de GRDs activos desde la base de datos."""
        # Limitar la frecuencia de consulta a la DB si no es un cambio critico
    ...
    def start_observer_loop(self):
        ...
        while True:
            self._refresh_grd_data() # Asegura que la lista de GRDs este actualizada
            ...
            # Asegurar conexion del driver al inicio del ciclo de monitoreo
            if not self.driver.is_connected() and not self.driver.connect():
                print("GRD Client: No se pudo establecer conexion con el servidor Modbus. Reintentando en el proximo ciclo.")
                time.sleep(self.refresh_interval)
                continue # Saltar al siguiente ciclo

            for grd_id in grd_ids_to_monitor:
                # Omitir el procesamiento para grd_id = 4
                if grd_id == 4:                    
                    continue
                ...
                if registers_data is not None and len(registers_data) >= self.register_count:
                    # El registro 16 esta en el indice 15 (0-indexed)
