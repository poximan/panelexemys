...
def populate_database_conditionally():
    """
    Puebla la tabla 'historicos' con datos históricos ficticios si config.POBLAR_BD es True.
    Genera un número de registros basado en HISTORICAL_DAYS_TO_GENERATE y
    ...
    # Esta llamada ya la hace `app.py` al inicio, pero no está de más tenerla aquí
    ...
    if config.POBLAR_BD:
        ...
        try:
            ...
            if num_grd_ids_to_use <= 0:
                print("ADVERTENCIA: config.GRD_DESCRIPTIONS está vacío. No se pueden generar datos ficticios para GRDs.")
                return

            # Calcular el número total de entradas deseado
            # Número de intervalos por día * número de días
            ...
            # Calcular el timestamp de inicio para los datos históricos
            start_time_for_historical_data = datetime.now() - timedelta(days=config.HISTORICAL_DAYS_TO_GENERATE)

            data_to_insert = []
            # Diccionarios para mantener el último estado y la última marca de tiempo por cada GRD.
            grd_last_states = {}
            ...
            print(f"Se generarán aproximadamente {total_insertions_desired} entradas ficticias para {num_grd_ids_to_use} GRDs, cubriendo {config.HISTORICAL_DAYS_TO_GENERATE} días.")
            ...
            for grd_id, description in grd_descriptions_map.items():
                # Asegura que el GRD ID exista en la tabla 'grd' con su descripción
                ...
                for _ in range(total_entries_per_grd):
                    # Avanza el tiempo según el intervalo configurado
            ...
            # Ordenar los datos por timestamp antes de la inserción masiva
            # Esto ayuda a evitar bloqueos y optimiza la inserción en algunas bases de datos
            ...
            ''', data_to_insert)
            conn.commit()
            print(f"Población completada: {cursor.rowcount} registros insertados en 'historicos'. Los duplicados existentes (si los hubo) fueron ignorados.")
        ...
        print("POBLAR_BD es False. No se poblará la base de datos con datos ficticios.")